...
// Rute Home (Untuk cek apakah API hidup)
app.get('/', (req, res) => {
    res.send('Server Mappi32 Aktif!');
});

// Rute Error (Cukup /api/error karena vercel.json mengarahkan ke sini)
app.get('/api/error', (req, res) => {
    client.publish('mappi32/api/perintah', 'Alarm 6');
    res.json({ status: "success", message: "Buzzer 6x sent" });
});

// Rute Warning
app.get('/api/warning', (req, res) => {
    client.publish('mappi32/api/perintah', 'WARNING_ON');
    res.json({ status: "success", message: "Warning LED sent" });
});
